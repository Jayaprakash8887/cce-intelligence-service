...
| `intelligenceEventId` | UUID | Yes | Compliance Service's `intelligence_event_log.id` (primary key). Idempotency anchor for `delivery_run`. |
...
| **Idempotency** | `(intelligenceEventId, channelSubscriptionId)` uniqueness on `delivery_run` |
