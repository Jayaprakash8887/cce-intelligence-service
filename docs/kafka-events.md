...
| `protocolDefinitionId` | UUID | Yes | FK to `protocol_definition.id` — routing key for `channel_subscription` lookup. Resolved by Compliance from `action_run.protocol_instance → protocol_instance.protocol_definition`. |
